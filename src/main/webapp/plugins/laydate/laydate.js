...
//
//laydate({
//    elem: '#datetimepicker',//目标元素。由于laydate.js封装了一个轻量级的选择器引擎，因此elem还允许你传入class、tag但必须按照这种方式 '#id .class'
...
$(function () {
    $('body').on('click', '.datePiker', function (e) {
        e.stopPropagation();
        laydate({targetObj:this,istime: true, format: 'YYYY-MM-DD'})
    });

    $('body').on('focus', '.datePiker', function (e) {
        e.stopPropagation();
        laydate({targetObj:this,istime: true, format: 'YYYY-MM-DD'})
    });
