  ...
  /*  ///////////响应下拉框选择
    ...
    var table = $('#studentsTable').mytable({
        ...
        operColFun:function(i,rowdata){
            var id =  rowdata['stuId'];
            var operObj = $('<i href="###" class="iconfont icon-viewlist bgColor-2" title="成绩证明-1" rownum="' + i + '" did="' + id + '"></i>');
            operObj.on('click', function () {
                if(typeof(winOpen)=="object"){
                    winOpen.close();             //关闭之前打开的窗口
                }
                winOpen=window.open(BAS_URL+"/scoExamScores/stuScoreCertificatePrint.do?id="+id,'stuScoreCertificatePrint');
            });
            return operObj;
        },
        operColFun_2:function(i,rowdata){
            var id =  rowdata['stuId'];
            var operObj = $('<i href="###" class="iconfont icon-viewlist bgColor-2" title="成绩证明-2" rownum="' + i + '" did="' + id + '"></i>');
            operObj.on('click', function () {
                if(typeof(winOpen)=="object"){
                    winOpen.close();             //关闭之前打开的窗口
                }
                winOpen=window.open(BAS_URL+"/scoExamScores/stuScoreCertificatePrintCO.do?id="+id,'stuScoreCertificatePrintCO');
            });
            return operObj;
        }
    });
