    ...
    function queryForData(params){
     ...
     //   var tempenrolYear="";
      //  var tempTerm="";
        ...
        var tempHtml_a;
        var tempHtml_b;
        var tempHtml='<div style="padding:0 20px 20px 20px;font-size:20px;font-weight:700;text-align: center;"><span>武汉音乐学院附属中等音乐学校学生总评成绩汇总表</span></div>';
        ...
        tempHtml=tempHtml+'<table style="width:100%;height: 100%;font-size: 13px;" cellpadding="1" class="tableOut"><tr><td style="width:100px;">序号</td><td style="width:150px;">学号</td>' +
            '<td style="width:150px;">姓名</td><td style="width:150px;">专业</td>';
        pageHeadForPrint=pageHeadForPrint+'<table style="width:100%;height: 100%;font-size: 13px;" cellpadding="1" class="tableOut"><tr><td style="width:100px;">序号</td><td style="width:150px;">学号</td>' +
        '<td style="width:150px;">姓名</td><td style="width:150px;">专业</td>';
        $.loadingBox.show();
        $.ajaxConnSend(this, 'subjectScores/doGetStuSubjectInfoListByParam.infc', params, function(data) {////////组建第一行
            ScoreSubjectListObj = data['object'];
            for(var i =0;i<ScoreSubjectListObj.length;i++){
                tempHtml=tempHtml+'<td style="width:150px;">'+ScoreSubjectListObj[i].subjectText+'</td>';
                pageHeadForPrint=pageHeadForPrint+'<td style="width:150px;">'+ScoreSubjectListObj[i].subjectText+'</td>';
            }
            tempHtml=tempHtml+'</tr>';
            pageHeadForPrint=pageHeadForPrint+'</tr>';
        }, function() {
            $.ajaxConnSend(this, 'subjectScores/doGetStuSubjectScoreInfoByParam.infc', params, function(data) {///////组建空表
                ScoreStuListObj = data['object'];
                console.debug(ScoreStuListObj);
                /////////////////////显示区内容
                tempHtml_a=tempHtml;
                var tempStId="";
                var lineNum=0;
                for(var i =0;i<ScoreStuListObj.length;i++){
                    if(tempStId!=ScoreStuListObj[i].stuId){  ///换行
                        ...
                        for(var j=0;j<ScoreSubjectListObj.length;j++){
                            tempHtml_a=tempHtml_a+'<td style="width:150px;"><span id="'+ScoreStuListObj[i].stuId+'_'+ScoreSubjectListObj[j].subjectId+'"></span></td>';
                        }
                ...
                for(var i =0;i<ScoreStuListObj.length;i++){
                    if(tempStId!=ScoreStuListObj[i].stuId){  ///换行
                        ...
                        '<td style="width:150px;">'+ScoreStuListObj[i].stuName+'</td><td style="width:150px;">'+ScoreStuListObj[i].majorText+'</td>';
                        for(var j=0;j<ScoreSubjectListObj.length;j++){
                            tempHtml_b=tempHtml_b+'<td style="width:150px;"><span id="'+ScoreStuListObj[i].stuId+'_'+ScoreSubjectListObj[j].subjectId+'_p'+'"></span></td>';
                        }
                        tempStId=ScoreStuListObj[i].stuId;
                    }else{
                ...
                $.ajaxConnSend(this, 'subjectScores/doGetStuSubjectScoreInfoByParam.infc', params, function(data) {///////填充数据
                    ScoreStuListObj = data['object'];
                    var tempStId="";
                    var lineNum=0;
                    for(var  k=0;k<ScoreStuListObj.length;k++){
                        $("#"+ScoreStuListObj[k].stuId+"_"+ScoreStuListObj[k].subjectId).empty();
                        $("#"+ScoreStuListObj[k].stuId+"_"+ScoreStuListObj[k].subjectId).html(ScoreStuListObj[k].score);
                        $("#"+ScoreStuListObj[k].stuId+"_"+ScoreStuListObj[k].subjectId+'_p').empty();
                        $("#"+ScoreStuListObj[k].stuId+"_"+ScoreStuListObj[k].subjectId+'_p').html(ScoreStuListObj[k].score);
                    }
                }, function() {
                    $.loadingBox.close();
                });
